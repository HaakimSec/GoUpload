...
func AllPayloads(techStack string) []*Payload {
	var all []*Payload

	// Universal payloads - work on all platforms
	all = append(all, moduleC()...) // Filename Obfuscation (NOTICE: ...)
	all = append(all, moduleE()...) // Size Boundary Testing (NOTICE: ...)
	all = append(all, moduleG()...) // Unicode & Encoding Attacks (NOTICE: ...)
